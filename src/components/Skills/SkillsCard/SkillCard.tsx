...
export default function SkillCard({ url, skill, showCirkle = true }: SkillCardProps) {
  ...
  return (
    <motion.div
      className={`relative flex items-center max-xs:justify-center gap-1 md:gap-2 md:w-[220px] max-xs:w-[120px] rounded-full border-2 border-[#1a7b81]  md:px-4 md:py-2 text-white ${styles.cardGlow} ${styles.skillCard}`}
      ...
      <div className="flex flex-col w-[90%] mt-[-5px] max-xs:items-center">
        <div className="flex items-center max-xs:justify-center  gap-1">
         { showCirkle && <span className="w-3 h-3 bg-[var(--color-active)] dark:bg-[var(--color-active-darker)] rounded-full max-xs:hidden"></span> }
          <span className={`max-xs:text-[10px] max-xs:justify-center  text-sm font-semibold text-[#af5900] dark:text-white ${showCirkle ? "" :"text-[10px] md:text-[13px] font-bold" }`}>{skill}</span>
        </div>
